...
def imresize(img, ratio):
    h, w = img.shape[0], img.shape[1]
    img = cv2.resize(img, (math.floor(w/ratio), math.floor(h/ratio)), cv2.INTER_NEAREST)
    if len(img.shape) == 2:
        img = img[:,:,np.newaxis]
    return img
...
def temp_denoise(model, noisyframe, sigma_noise):
    ...
    ----------
        ...
        noisyframe  : Expected to be normalized in [0., 1.].
        sigma_noise : Noise level (standard deviation).
    
    Returns
    -------
        out         : Denoised sequence.
    '''
    # make size a multiple of 8
    sh_im = noisyframe.size()
    expanded_h = sh_im[-2]%8
    if expanded_h:
        expanded_h = 8-expanded_h
    expanded_w = sh_im[-1]%8
    if expanded_w:
        expanded_w = 8-expanded_w
...
def denoise_seq(seq, noise_std, temp_psz, model_temporal):
    '''
    Denoises a sequence of frames with a specified denoising model.
